...
export default function SeatWarning({ policy, onAcceptSuggestion, disabled = false }: Props) {
  ...
  return <div className='ticket-notice ticket-notice-warning'>
    <div role='status' aria-live='polite'>
      <p>{policy.notice}</p>
      {suggestion ? <p className='ticket-recommendation'><strong>Empfehlung: {suggestion.map(seatLabel).join(' · ')}</strong><br/>Alle nebeneinander, gleiche Anzahl, ohne neue Einzellücken.</p>
        : <p className='ticket-recommendation'>Für diese Platzanzahl ist aktuell keine zusammenhängende Alternative ohne neue Einzellücken verfügbar.</p>}
    </div>
