...
export async function POST(request:Request) {
  ...
  try {
    ...
    const result=await env.DB.prepare("UPDATE bookings SET status = ?, checked_in_at = datetime('now'), version = version + 1, wallet_sync_pending = 1 WHERE id = ? AND status = ?")
      .bind('checked_in',bookingId,'confirmed').run()
